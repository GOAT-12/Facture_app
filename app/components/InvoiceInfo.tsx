...
const InvoiceInfo: React.FC<Props> = ({invoice, setInvoice}) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, filed : string) => {
    setInvoice({...invoice, [filed]: e.target.value})
  }
  ...
  return (
    <div className='flex flex-col h-fit bg-base-200 p-5 rounded-xl mb-4 md:mb-0'>
       <div className='space-y-4'>
          <h2 className='badge badge-info'>Emetteur</h2>
          <input type="text"  value={invoice?.issuerName} placeholder="Nom de l'entreprise émettrice"
          ...
          <input type="text"  value={invoice?.clientName} placeholder="Nom de l'entreprise Cliente"
          className='input input-bordered w-full resize-none'
          required
          ...
          <h2 className='badge badge-info'>Date de facture</h2>
