...
import { Check, Clock, FileText, SquareArrowOutUpRight, XCircle } from 'lucide-react';
...
const invoiceComponent: React.FC<InvoiceComponentProps> = ({invoice, index}: InvoiceComponentProps) => {
...
const calculateTotal = () =>{
  ...
  return (
    <div className='bg-base-200/90 p-5 rounded-xl space-y-2 shadow-xl'>
      <div className='flex justify-between items-center w-full'>
        <div>{getStatusBadge(invoice.status)}</div>
        <Link href={`invoice/${invoice.id}`} className='btn btn-info btn-sm'>
            plus <SquareArrowOutUpRight className='w-4 h-4'/>
        </Link>
      </div>
